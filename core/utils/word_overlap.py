...
def get_diferences(s: Document, r: Document):
    source_words = get_words(s)
    source_content_words = get_content_words(s)
    target_words = get_words(r)
    target_content_words = get_content_words(r)

    diff_source_to_target_content_words = [w for w in source_content_words if not w in target_content_words]
    diff_source_to_target = [w for w in source_words if not w in target_words]
    diff_target_to_source_content_words = [w for w in target_content_words if not w in source_content_words]
    diff_target_to_source = [w for w in target_words if not w in source_words]
...
def get_overlap(l1, l2):
    ...
    if len(l1) == 0:
        return 0
    return len([w for w in l1 if w in l2]) / len (l1)
